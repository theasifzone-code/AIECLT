...
function Register() {
  ...
  const loadCenters = async (city = '') => {
    try {
      ...
      const { data } = await api.get('/ocr/centers', {
        params: { city, limit: 200 },
      });
      setCenters(data.centers || []);
    } catch (err) {
      console.error('Failed to load centers:', err);
      // Silent fail — user manually center code daal sakta
    } finally {
  ...
  const loadCities = async () => {
    try {
      const { data } = await api.get('/ocr/cities');
  ...
  const handleCityChange = (city) => {
    setCityFilter(city);
    setFormData({ ...formData, examCenter: '', city });
    loadCenters(city);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ Validation: Student must select center
    if (!formData.examCenter) {
      setLocalError('Please select your exam center');
      return;
    }

    if (!formData.rollNumber) {
      setLocalError('Please enter your roll number');
      return;
    }

    if (!formData.grade) {
      setLocalError('Please select your grade/class');
      return;
    }
  ...
  return (
    <div
      className={`min-h-screen w-full flex items-center justify-center p-0 lg:p-6 transition-colors duration-300 ${
        theme === 'dark' ? 'bg-gray-950 text-white' : 'bg-slate-100 text-slate-900'
      }`}
    >
      <div
        className={`w-full max-w-5xl min-h-[650px] grid grid-cols-1 lg:grid-cols-12 shadow-2xl overflow-hidden transition-all lg:rounded-3xl border ${
          theme === 'dark'
      ...
      >
        {/* Left Panel */}
        <div
          className={`hidden lg:flex lg:col-span-5 p-10 flex-col justify-between relative overflow-hidden ${
            theme === 'dark'
        ...
        >
          ...
          <div>
            <Link to="/" className="inline-flex items-center gap-3 group focus:outline-none">
              ...
              <div className="flex flex-col text-left">
                <span className="text-xl font-black tracking-tight text-white">AI-ECLT</span>
          ...
          <div className="my-auto space-y-6 py-6">
            ...
            <p className="text-sm text-blue-100/80 leading-relaxed font-normal">
              Register now to track your exam centers, check seat allocations, and receive
              AI-optimized schedules effortlessly.
            </p>
        ...
        <div className="lg:col-span-7 p-6 sm:p-10 flex flex-col justify-between">
          <div>
            ...
            <div
              className={`p-3.5 rounded-2xl mb-5 text-xs font-medium flex items-start gap-3 border ${
                theme === 'dark'
            ...
            >
              ...
              <div>
                ...
                >
                  Board Officials and Admins are created directly by the Super Admin. Only
            ...
            {(localError || error) && (
              <div className="bg-red-500/10 border border-red-500/30 text-red-500 dark:text-red-400 p-3 rounded-xl mb-4 text-xs font-medium flex items-center gap-2.5">
            ...
            {success && (
              <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 p-3 rounded-xl mb-4 text-xs font-medium flex items-center gap-2.5">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className={`w-full px-4 py-3 rounded-xl border text-xs font-medium outline-none transition ${
                      theme === 'dark'
                        ? 'bg-gray-800/50 border-gray-700 text-white focus:border-blue-500'
                        : 'bg-slate-50/80 border-slate-200 text-slate-900 focus:border-blue-600'
                    }`}
                  >
                    <option value="">All Cities</option>
                    {cities.map((c) => (
                      <option key={c.city || c} value={c.city || c}>
                        {c.city || c} {c.centerCount ? `(${c.centerCount})` : ''}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div>
                ...
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    <BuildingOfficeIcon className="h-4 w-4 text-gray-400 group-focus-within:text-blue-500 transition" />
                  </div>
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setFormData({
                        ...formData,
                        examCenter: centerId,
                        city: center?.city || formData.city,
                      });
                    }}
                    className={`w-full pl-10 pr-4 py-3 rounded-xl border text-xs font-medium outline-none transition ${
                      theme === 'dark'
                        ? 'bg-gray-800/50 border-gray-700 text-white focus:border-blue-500'
                        : 'bg-slate-50/80 border-slate-200 text-slate-900 focus:border-blue-600'
                    }`}
                  ...
                  >
                    <option value="">
                      {loadingCenters ? 'Loading centers...' : 'Select your exam center'}
                    </option>
                    {centers.map((c) => (
                ...
                {centers.length === 0 && !loadingCenters && (
                  <p className="text-[10px] mt-1 text-amber-500 font-medium">
                    ⚠️ No centers found. Please select a city first or contact admin.
                  </p>
                )}
              </div>

              {/* ============ ROLL NUMBER + BOARD (Row) ============ */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label
                    className={`block mb-1.5 text-xs font-bold uppercase tracking-wider ${
                  ...
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                      <HashtagIcon className="h-4 w-4 text-gray-400 group-focus-within:text-blue-500 transition" />
                    </div>
                    <input
                      type="text"
                      placeholder="12345"
            ...
            <div className="relative my-5">
              ...
              <div className="relative flex justify-center text-[10px]">
                <span
                  className={`px-3 font-bold uppercase tracking-wider ${
                    theme === 'dark'
